...
Runnable = (function(){
		...
		/**
		* @constructor
		* @param {String} config.modulePath: module path see child_process.spawn params
		* @param {Array} config.args: see child_process.spawn params
		...
		*/
		function Runnable(config) {
			...
			this.args.unshift(this.modulePath);
		
			this.options = { // see child_process.spawn params
				cwd: process.cwd(),
				env: process.env,
				stdio: [0, 1, 2, 'ipc']
			};
			...
			this._busy = false;
			this._spawn = cp.spawn(process.argv[0], this.args, this.options);
			this._spawn.on('message', this._onRunnableMessage.bind(this));
			this._spawn.on('exit', this._onRunnableExit.bind(this));
			this._spawn.on('close', this._onRunnableClose.bind(this));
			this._spawn.on('disconnect', this._onRunnableDisconnect.bind(this));
			this._runCount = 0;
			this._timeoutId = -1;
		...
		*/
		...
		Runnable.prototype._log = function() {
			...
			var 
			args = arguments,
			v = util.format('%d Runnable %d# ', parseInt((new Date()).getTime(), 10), this._spawn.pid);
		...
		*/
		Runnable.prototype._onTimeout = function() {
			...
			this._spawn.removeAllListeners('exit');
			this._spawn.on('exit', this._onKilled.bind(this));
			this._spawn.kill();
		};
		...
		*/
		Runnable.prototype._onKilled = function(exitCode, signal) {
			...
			this._eexception({
					code: 'ETIMEOUT',
					pid: this._spawn.pid,
					message: util.format('Run timeout (%ds). Process killed (%d)', this.timeout, this._spawn.pid)
			});
		};
...
exports.RunnablePool = (function() {
		...
		*/
		...
		util.inherits(RunnablePool, events.EventEmitter);
		...
		*/
		...
		/**
		* @public
		* This method clean the pooler
		*/
		RunnablePool.prototype.abort = function() {
			this._runnableStack = [];
		};
		...
		RunnablePool.prototype._getFreeRunnable = function(){
			...
			if (this._freeRunnables.length > 0) {
				...
				this._busyRunnables[freeRunnable._spawn.pid] = freeRunnable;
		...
		RunnablePool.prototype._createRunnable = function() {
			...
			this._log('New runnable created. pid: %d', runnable._spawn.pid);
	
			this._busyRunnables[runnable._spawn.pid] = runnable;
			this._busyRunnables.count++;
		...
		RunnablePool.prototype._disconnectRunnables = function(){
			...
			while (this._freeRunnables.length > 0) {
				...
				pid = runnable._spawn.pid;
				runnable._spawn.on('exit', onExit.bind(this));
